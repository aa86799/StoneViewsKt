...
class TabLayoutFragment : BaseFragment() {
    ...
    override fun onPreparedView(savedInstanceState: Bundle?) {
        ...
        for (tab in TABS) {
            ...
            fragment_tab_layout_tl2.addTab(fragment_tab_layout_tl2.newTab().setText(tab))
        ...
        fragment_tab_layout_tl3.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            ...
            override fun onTabUnselected(tab: TabLayout.Tab) {
                ((tab.customView as LinearLayout).getChildAt(0) as TextView).textColor = Color.BLACK
            }

            override fun onTabSelected(tab: TabLayout.Tab) {
                ((tab.customView as LinearLayout).getChildAt(0) as TextView).textColor = Color.GREEN
            }
        })
...
//        fragment_tab_layout_tl3.getTabAt(0)?.select()
        //强行设置第0个
        ((fragment_tab_layout_tl3.getTabAt(0)?.customView as LinearLayout).getChildAt(0) as TextView).textColor = Color.GREEN
    }

    private fun makeTabView(titles: List<String>, position: Int): View {
        //如下居然没效果，在测试机上，见鬼了
//        val tabView: View = LayoutInflater.from(context).inflate(R.layout.layout_tab_icon, null)
