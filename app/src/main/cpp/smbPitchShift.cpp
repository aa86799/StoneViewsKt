...
Java_com_stone_stoneviewskt_ui_audio_AudioPitchProcessor_process(JNIEnv *env, jobject thiz,
        jfloat ratio, jbyteArray input_, jbyteArray output_, jint size, jint sampleRate, jfloatArray float_input_, jfloatArray float_output_) {
    //将输入输出参数，转成指针
    jbyte *input = env -> GetByteArrayElements(input_, nullptr);
    jbyte *output = env -> GetByteArrayElements(output_, nullptr);
    jfloat *float_input = env -> GetFloatArrayElements(float_input_, nullptr);
    jfloat *float_output = env -> GetFloatArrayElements(float_output_, nullptr);

    //将 input 转成 float array
    for (int i = 0; i < size; i += 2) {
        int lower = input[i] & 0xFF;
        int higher = input[i + 1] & 0xFF;
        int frame = (higher << 8) + lower;
        float_input[i >> 1] = (signed short) frame;

    }

    //call smbPitchShift() 变化音高
    smbPitchShift(ratio, 1024, 1024, 4, sampleRate, float_input, float_output);

    //将 处理后的 float array 转成 byte array
//    int length = env->GetArrayLength(float_input_);
//    for (int i = 0; i < length; ++i) {
  for (int i = 0; i < size; i += 2) {
        int frame = (int) float_output[i >> 1];
        output[i] = (jbyte) (frame & 0xff);
        output[i + 1] = (jbyte) (frame >> 8);
    }

    //释放 array对应的 指针；入参 array 不受影响
    env->ReleaseByteArrayElements(input_, input, 0);
    env->ReleaseByteArrayElements(output_, output, 0);
    env->ReleaseFloatArrayElements(float_input_, float_input, 0);
    env->ReleaseFloatArrayElements(float_output_, float_output, 0);
}